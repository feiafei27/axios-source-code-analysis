...
Axios.prototype.request = function request(config) {
  ...
  // 1.首先是请求过滤器函数,请求过滤器函数的特点是传参是config，返回也是config，由于promise的特点，
  // 返回的config会被包装成成功的promise，以便执行下一个.then,下一个.then又会执行下一个请求过滤器函数，
  // 直至执行到请求处理函数，也就是 dispatchRequest
  // 2.当promise执行到 dispatchRequest 的时候，该函数的参数是上一个请求过滤器返回的config，该函数会获取指定的adapter，
